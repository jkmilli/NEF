NefariousCoin-qt: Qt4 GUI for NefariousCoin
===============================
...
Alternatively, install Qt Creator and open the `NefariousCoin-qt.pro` file.

An executable named `NefariousCoin-qt` will be built.
...
A warning for people using the *static binary* version of NefariousCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of NefariousCoin is linked against libdb5.3.
...
installed causes NefariousCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
