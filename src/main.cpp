...
unsigned int nStakeTargetSpacing = 8.33 * 60; // 8.33-minute stakes spacing
int64_t nTargetSpacing = 8.33 * 60;  // Same as the above
...
const string strMessageMagic = "NefariousCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(unsigned int nBits, int64_t nFees, int nHeight)
{
	...
	if (nHeight == 1)
		return 950000000 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 2500000;
    if (halvings >= 64)
        return nFees;
...
static const int64_t nTargetTimespan = 8.33 * 15 * 60;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("NefariousCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in NefariousCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // NefariousCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NefariousCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "NEF Genesis";
        CTransaction txNew;
        txNew.nTime = 1488234318;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1488234318;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 235653; // !fTestNet ? 1575379 : 46534;
...
unsigned char pchMessageStart[4] = { 0x44, 0xee, 0xd2, 0xe5 };
