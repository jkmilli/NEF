...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("NefariousCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NefariousCoinTEST2\r");
            Send(hSocket, "WHO #NefariousCoinTEST2\r");
        } else {
            // randomly join #NefariousCoin00-#NefariousCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #NefariousCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NefariousCoin%02d\r", channel_number).c_str());
        }
